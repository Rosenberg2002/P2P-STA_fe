...
template <class K, class V>
xMap<K, V>::xMap(int (*hashCode)(K &, int), float loadFactor = 0.75,
    bool (*valueEqual)(V &, V &) = nullptr,
    void (*deleteValues)(xMap<K, V> *) = nullptr,
    bool (*keyEqual)(K &, K &) = nullptr,
    void (*deleteKeys)(xMap<K, V> *) = nullptr)
  : hashCode(hashCode), loadFactor(loadFactor), valueEqual(valueEqual), deleteValues(deleteValues), keyEqual(keyEqual), deleteKeys(deleteKeys) {
  count = 0;
  capacity = 10;
...
V xMap<K, V>::put(K key, V value) {
  ...
  V retValue = value;
  DLinkedList<Entry *> &temp = table[index];
...
template <class K, class V>
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K) = nullptr) {
  int index = hashCode(key, capacity);
  DLinkedList<Entry *> &temp = table[index];

  for (int i = 0; i < temp.size(); ++i) {
    Entry *entry = temp.get(i);
    if (keyEQ(entry->key, key)) {
...
bool xMap<K, V>::containsValue(V value) {
  for (int i = 0; i < capacity; ++i) {
    DLinkedList<Entry *> &temp = table[i];
    for (Entry *entry : temp) {
